...
def main():
    ...
    try:
        ...
        rospy.logerr("No Serial0 device found!")
        connected = False
        pass

    try:
        ser1 = serial.Serial('/dev/ttyUSB1', 115200, timeout=1)
        rospy.logwarn("Using serial1 interface: %s", ser1.name)
        connected1 = True
    except Exception:
        rospy.logerr("No Serial1 device found!")
        connected = False
        pass
    ...
    while not rospy.is_shutdown():
        ...
        binR_dir = struct.pack('h', 0)
        binR = struct.pack('h', int(motorR))
        binL_dir = struct.pack('h', 0)
        binL = struct.pack('h', int(motorL))
